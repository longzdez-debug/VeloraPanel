...
from core.process_policy import ProcessPolicy
from core.process_supervisor import make_binding, wait_for_cs2_child
...
class Account:
    def __init__(self, login, password, shared_secret=None, steam_id = 0, identity_secret=None):
        ...
        affinity = self._settingsManager.get("ProcessAffinity", [])
        self._process_policy = ProcessPolicy(
            priority=self._settingsManager.get("ProcessPriority", "normal"),
            ...
            try:
                ...
                if entry:
                    ...
                    if psutil.pid_exists(steam_pid) and psutil.pid_exists(cs2_pid):
                        ...
                        if (
                            cs2_proc.name().lower() == "cs2.exe"
                            and cs2_proc in steam_proc.children(recursive=True)
                            and (
                                entry.get("SteamCreateTime") is None
                                or abs(steam_proc.create_time() - float(entry["SteamCreateTime"])) <= 0.01
                            )
                            and (
                                entry.get("CS2CreateTime") is None
                                or abs(cs2_proc.create_time() - float(entry["CS2CreateTime"])) <= 0.01
                            )
                        ):
    ...
    def StartGame(self):
        time.sleep(5)
        print("Запуск Steam...")
        steam_path = self._settingsManager.get("SteamPath", r"C:\Program Files\Steam\steam.exe")
        ...
        try:
            ...
            args = (
                f'{self._settingsManager.get("SteamArg", "-nofriendsui -nochatui -language english")}'
                f' -applaunch 730 '
        ...
        # Логин + ожидание CS2
        cs2_executable = str(Path(cs2_path) / "game" / "bin" / "win64" / "cs2.exe")
        launch_timeout = max(
            30,
            int(self._settingsManager.get("CS2LaunchTimeout", 180) or 180),
        )
        self.ProcessWindowsBeforeCS(self.steamProcess.pid)
        cs2_process = wait_for_cs2_child(
            self.steamProcess.pid,
            timeout=launch_timeout,
            cs2_executable=cs2_executable if Path(cs2_executable).exists() else None,
        )
        if cs2_process is None:
            print(
                f"❌ [{self.login}] CS2 не запустился за {launch_timeout} секунд; "
                "останавливаем только принадлежащий Steam"
            )
            self.KillSteamAndCS()
            return

        self.CS2Process = cs2_process
        if self._process_policy.apply(cs2_process):
            self._logManager.add_log(
                f"✅ [{self.login}] process policy applied to CS2"
            )
        self._kill_cs2_mutex(cs2_process.pid)
        ...
        try:
            ...
            binding = make_binding(self.steamProcess, self.CS2Process)
            data.append({
                "login": self.login,
                "SteamPid": binding.steam_pid,
                "SteamCreateTime": binding.steam_create_time,
                "CS2Pid": binding.cs2_pid,
                "CS2CreateTime": binding.cs2_create_time,
            })
