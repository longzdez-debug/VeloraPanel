...
class TelegramBotManager:
    ...
    def _is_authorized(self, chat_id):
        # An empty allowlist is fail-closed: remote control must be explicitly enabled.
    ...
    def _handle_message(self, message):
        ...
        if not self._is_authorized(chat_id):
            self._log(f"⚠️ Telegram unauthorized chat rejected: {chat_id}")
            return
    ...
    def _handle_callback_query(self, callback_query):
        ...
        if not chat_id:
            return
        if not self._is_authorized(chat_id):
            self._answer_callback(callback_id, "Доступ запрещён")
            self._log(f"⚠️ Telegram unauthorized callback rejected: {chat_id}")
            return
