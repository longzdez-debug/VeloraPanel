...
class LobbyManager:
    ...
    def MakeLobbiesAndSearchGame(self):
        ...
        try:
            max_cycles = max(1, int(self._settingManager.get("SearchRetriesBeforeShuffle", 3)))
            search_timeout = max(30, int(self._settingManager.get("GameSearchTimeout", 600)))
        except (TypeError, ValueError):
            max_cycles = 3
            search_timeout = 600
        ...
        self._logManager.add_log(
            f"❌ Match was not found after {max_cycles} recovery cycles"
        )
        return False