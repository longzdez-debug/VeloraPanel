...
try {
    ...
    console.log(`HandleError Invalid stdin payload ${err.message}`);
    process.exit(20);
}

if (!credentials || !credentials.login || !credentials.password ||
    !credentials.shared_secret || !credentials.identity_secret ||
    !credentials.tradeOfferLink) {
    console.log("HandleError Not enough credentials");
    process.exit(20);
}

const login = credentials.login;
const password = credentials.password;
const shared_secret = credentials.shared_secret;
const identity_secret = credentials.identity_secret;
const tradeOfferLink = credentials.tradeOfferLink;
let inventoryString = credentials.inventoryString || "730/2";
const dryRun = credentials.dryRun === true;
const maxItems = Number.isInteger(credentials.maxItems) && credentials.maxItems > 0
    ? credentials.maxItems
    : 0;
...
function archivePreviousWeekReport(savedState, reportPath, reportDataPath) {
    ...
    try {
        if (fs.existsSync(reportPath)) {
            ...
            fs.writeFileSync(archivePath, renderVeloraReport(savedState), 'utf8');
        }
...
function renderVeloraReport(state) {
    const reportLines = [];
...
async function buildAndWriteReport(sentItems, currentLogin) {
    ...
    fs.writeFileSync(reportPath, renderVeloraReport(reportState), 'utf8');
}
                    ...
                    manager.getInventoryContents(
                        ...
                        function(err, inventoryItems) {
                            ...
                            if (err) {
                                ...
                                inventoryItems.slice(0, maxItems > 0
                                    ? Math.max(0, maxItems - totalItemsCount)
                            ...
                            if (processedInventories === inventoriesToSend.length) {
                                ...
                                if (dryRun) {
                                    console.log(`${tradeName}: DRY_RUN enabled; offer was not sent`);
                                    sentOfferCount++;
                                    completedTrades++;
                                    if (completedTrades === totalTrades) {
                                        finalizeExit();
