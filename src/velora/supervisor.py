...
class Supervisor:
 ...
 def __post_init__(self):
  ...
  self.processes=ProcessSupervisor();self.launcher=Cs2Launcher(self.processes)
 ...
 def _bind_walkbot(self,a):
  def replan(position):
   if not a.route_map or not a.route_goal:return False
   graph=self.route_store.get(a.route_map)
   path=graph.path_from_position(position,a.route_goal,max_snap_distance=1200)
   if not path:return False
   a.walkbot.set_path(path);return True
  a.walkbot.replan=replan

 def get_account(self,account_id):return next((x for x in self.accounts if x.id==account_id),None)

 def on_gsi(self,snap):
  for a in self.accounts:a.on_gsi(snap)
  for a in self.accounts:
   if a.route_map and snap.map_name and snap.map_name!=a.route_map:a.walkbot.input.release_all()
 ...
 def set_route_from_position(self,account_id,map_name,goal,position):
  a=self.get_account(account_id)
  if not a:raise KeyError(account_id)
  graph=self.route_store.get(map_name);path=graph.path_from_position(position,goal,max_snap_distance=1200)
  if not path:raise ValueError("no route from current position")
  a.walkbot.set_path(path);a.route_map=map_name;a.route_start=path[0].id;a.route_goal=goal
  return path
 ...
 def start_account(self,account_id):
  ...
  try:
   ...
   a.process_id=r.identity.pid;a.executable=r.executable
  except Exception as e:
   a.errors.append(str(e))
   if a.fsm.state!=AccountState.ERROR:a.fsm.dispatch("error")
   a.walkbot.stop();raise
   ...
   while self.running:
    for a in self.accounts:
     if a.process_id and not self.processes.alive(a.process_id):
      a.process_id=None;a.walkbot.emergency_stop();a.errors.append("CS2 process exited")
