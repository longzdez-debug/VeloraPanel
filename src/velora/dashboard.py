...
function accountRow(x,check=false){return '<div class=acct><div class=acctmain>'+(check?'<input type=checkbox data-account="'+esc(x.id)+'">':'')+'<div class=avatar>'+initials(x.name)+'</div><div><b>'+esc(x.name)+'</b><small>'+esc(x.id)+'</small></div></div><span class="state '+stateClass(x.state)+'">● '+esc(x.state)+'</span><div class="cell hide-md">MATCH<b>'+esc(x.match)+'</b></div><div class="cell hide-md">WALKBOT<b>'+esc(x.walkbot)+'</b></div><div class=actions-sm><button onclick="act(\''+jsesc(x.id)+'\',\'start\')">START</button><button onclick="act(\''+jsesc(x.id)+'\',\'stop\')">STOP</button><button class=danger onclick="act(\''+jsesc(x.id)+'\',\'kill\')">KILL</button></div></div>'}
...
async function diag(){try{lastDiag=await api('/api/diagnostics');$('diagRaw').textContent=JSON.stringify(lastDiag,null,2);$('diagGrid').innerHTML=(Array.isArray(lastDiag)?lastDiag:[]).map(v=>'<div><small>'+esc(String(v.name||'CHECK').replaceAll('_',' ').toUpperCase())+'</small><b class="'+(v.ok?'ok':'bad')+'">'+(v.ok?'● PASS':'● FAIL')+'</b><small style="display:block;margin-top:5px">'+esc(v.detail||'')+'</small></div>').join('')||'<div class=muted>No diagnostic data.</div>';$('healthGrid').innerHTML=$('diagGrid').innerHTML}catch(e){$('diagRaw').textContent=e.message}}
...
async function loadLanguage(){try{$('language').value=(await api('/api/settings/language')).language||'en'}catch{}}
