...
<div class=content><header class=top><div class=crumb>VELORA / <b id=pageTitle>Dashboard</b></div><div class=actions><span id=topState class="pill run">RUNNING</span><button onclick=diag()>Diagnostics</button><button class=danger onclick=kill()>EMERGENCY STOP ALL</button></div></header><main class=main>
...
function renderRoute(){const c=$('canvas');c.innerHTML='<div class=center>ROUTE GRAPH / '+route.nodes.length+' NODES · '+route.edges.length+' EDGES</div>';route.nodes.forEach((n,i)=>{const e=document.createElement('button');e.className='node';e.style.left=(12+(i*41)%78)+'%';e.style.top=(15+(i*59)%72)+'%';e.title=n.id;e.onclick=()=>{document.querySelectorAll('.node').forEach(x=>x.classList.remove('sel'));e.classList.add('sel');$('inspector').innerHTML='<b>'+n.id+'</b><br><br>X '+n.x+'<br>Y '+n.y+'<br>Z '+n.z;$('del').value=n.id;$('nid').value=n.id};c.appendChild(e)});$('routeInfo').textContent=route.nodes.length+' nodes · '+route.edges.length+' edges'}
