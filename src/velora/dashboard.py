...
def _as_bool(value, field="value"):
...
<section id=fsm class=page><div class=hero><div><div class=eyebrow>STATE MACHINE</div><h1>Account FSM</h1><p>Live account lifecycle states across all workers.</p></div></div><div class=panel><div class=ph><b>ACCOUNT LIFECYCLE</b><small>Live snapshot</small></div><div class=pb><div id=fsmBoard class=fsm></div></div></div><div class="panel" style="margin-top:13px"><div class=ph><b>STATE TABLE</b><div class=toolbar><select id=fsmAccount oninput=renderFsmHistory()></select><select id=fsmHistoryType onchange=renderFsmHistory()><option value=fsm>Account FSM</option><option value=match>Match FSM</option><option value=walkbot>WalkBot FSM</option></select></div></div><div class=pb><table class=table><thead><tr><th>ACCOUNT</th><th>STATE</th><th>RESTARTS</th><th>PID</th><th>STARTED</th></tr></thead><tbody id=fsmTable></tbody></table><div id=fsmHistory style="margin-top:13px"></div></div></div></section>
...
'EMERGENCY CONTROL':'АВАРИЙНОЕ УПРАВЛЕНИЕ','Global kill switch and recovery controls':'Глобальная аварийная остановка и средства восстановления','KILL SWITCH':'АВАРИЙНАЯ ОСТАНОВКА',
'STOP ALL':'ОСТАНОВИТЬ ВСЁ','Emergency Stop All':'Аварийная остановка всех процессов','Immediately stop active batches and automation workers.':'Немедленно остановить активные задания и процессы автоматизации.',
'EMERGENCY STOP':'АВАРИЙНАЯ ОСТАНОВКА','Emergency Stop':'Аварийная остановка','Stop all active batches, FSM workers and WalkBot processes?':'Остановить все активные задания, FSM-воркеры и процессы WalkBot?',
'RUNNING':'РАБОТАЕТ','SYSTEM ONLINE':'СИСТЕМА В СЕТИ','SYSTEM STOPPED':'СИСТЕМА ОСТАНОВЛЕНА','KILL SWITCH ACTIVE':'АВАРИЙНАЯ ОСТАНОВКА АКТИВНА','CLEAR KILL SWITCH':'СБРОСИТЬ АВАРИЙНУЮ ОСТАНОВКУ',
'registered accounts':'зарегистрированных аккаунтов','active account workers':'активных воркеров аккаунтов','orchestration jobs':'заданий оркестрации','active workers':'активных воркеров',
'MANAGE ACCOUNTS →':'УПРАВЛЕНИЕ АККАУНТАМИ →','RUN DIAGNOSTICS':'ЗАПУСТИТЬ ДИАГНОСТИКУ','VIEW ALL →':'ПОКАЗАТЬ ВСЕ →','CHECK':'ПРОВЕРИТЬ','REFRESH':'ОБНОВИТЬ',
'CREATE BATCH':'СОЗДАТЬ ЗАДАНИЕ','SAVE MAP':'СОХРАНИТЬ КАРТУ','RAW OUTPUT':'РЕЗУЛЬТАТ ПРОВЕРКИ','CANCEL':'ОТМЕНА','CONFIRM':'ПОДТВЕРДИТЬ','CLEAR':'ОЧИСТИТЬ',
'START':'ЗАПУСТИТЬ','STOP':'ОСТАНОВИТЬ','KILL':'ЗАВЕРШИТЬ','RECOVER':'ВОССТАНОВИТЬ','DELETE':'УДАЛИТЬ',
'Manage account workers, routes and process lifecycle.':'Управление воркерами аккаунтов, маршрутами и жизненным циклом процессов.',
'All states':'Все состояния','Running':'Работает','Error':'Ошибка','Stopped':'Остановлен','Manual':'Вручную','2v2':'2v2','2v2 Random':'2v2 — случайный','5v5':'5v5','5v5 Shuffle':'5v5 — случайный','Deathmatch':'Бой насмерть','Arms Race':'Гонка вооружений','Armory':'Оружейная',
'Batches':'Задания','Match jobs, readiness and recovery state.':'Матчевые задания, готовность и состояние восстановления.',
'Account FSM':'FSM аккаунтов','Live account lifecycle states across all workers.':'Текущие состояния жизненного цикла всех воркеров.',
'ACCOUNT LIFECYCLE':'ЖИЗНЕННЫЙ ЦИКЛ АККАУНТА','Live snapshot':'Текущее состояние','STATE TABLE':'ТАБЛИЦА СОСТОЯНИЙ','ACCOUNT':'АККАУНТ','STATE':'СОСТОЯНИЕ','RESTARTS':'ПЕРЕЗАПУСКИ','PID':'PID','STARTED':'ЗАПУЩЕН','TRANSITION':'ПЕРЕХОД','No states.':'Нет состояний.','No transition history.':'Нет истории переходов.',
'Match FSM':'FSM матчей','Match search, gameplay and result telemetry.':'Поиск матча, игровой процесс и телеметрия результата.','ROUND':'РАУНД','SCORE':'СЧЁТ','RESULT':'РЕЗУЛЬТАТ','OPPONENT':'СОПЕРНИК','No match data.':'Нет данных о матчах.',
'WalkBot':'WalkBot','Navigation state, route target and telemetry age.':'Состояние навигации, цель маршрута и давность телеметрии.','ROUTE':'МАРШРУТ','GOAL':'ЦЕЛЬ','TARGET':'ЦЕЛЬ','DISTANCE':'РАССТОЯНИЕ','NORMAL':'НОРМА','RECOVERY ACTIVE':'ВОССТАНОВЛЕНИЕ АКТИВНО','OFFLINE':'НЕ В СЕТИ','No WalkBot workers.':'Нет активных воркеров WalkBot.',
'Game State Integration':'Игровая телеметрия','Connection age and process telemetry for every account.':'Состояние подключения и телеметрия процесса для каждого аккаунта.','GSI ONLINE':'GSI В СЕТИ','GSI OFFLINE':'GSI НЕ В СЕТИ','telemetry streams':'потоков телеметрии','no recent event':'нет свежих событий','PROCESS PIDS':'PID ПРОЦЕССОВ','active process handles':'активных процессов','ROUND EVENTS':'СОБЫТИЯ РАУНДОВ','observed rounds':'зафиксированных раундов','SERVER':'СЕРВЕР','PACKETS':'ПАКЕТЫ','ERRORS':'ОШИБКИ','STREAMS':'ПОТОКИ','GSI connection':'Подключение GSI','received packets':'получено пакетов','packet errors':'ошибок пакетов','active streams':'активных потоков',
'Route Editor':'Редактор маршрутов','Build and validate WalkBot navigation graphs.':'Создание и проверка графов навигации WalkBot.','NEW MAP':'НОВАЯ КАРТА','ROUTE GRAPH':'ГРАФ МАРШРУТА','TOOLS':'ИНСТРУМЕНТЫ','Add waypoint':'Добавить точку','Connect nodes':'Соединить узлы','Delete node':'Удалить узел','NODE ID':'ID УЗЛА','X / Y / Z':'X / Y / Z','ADD / UPDATE NODE':'ДОБАВИТЬ / ОБНОВИТЬ УЗЕЛ','NODE INSPECTOR':'ИНСПЕКТОР УЗЛА','Select a node.':'Выберите узел.','CONNECT FROM':'ИЗ УЗЛА','CONNECT TO':'В УЗЕЛ','CONNECT':'СОЕДИНИТЬ','DELETE NODE':'УДАЛИТЬ УЗЕЛ','NO NODES · ADD WAYPOINT':'НЕТ УЗЛОВ · ДОБАВЬТЕ ТОЧКУ','VALIDATE':'ПРОВЕРИТЬ','NOT VALIDATED':'НЕ ПРОВЕРЕНО',
'Diagnostics':'Диагностика','Runtime, Steam, CS2, ports and data directory checks.':'Проверка среды выполнения, Steam, CS2, портов и каталога данных.','SYSTEM':'СИСТЕМА','PASS':'ПРОЙДЕНО','FAIL':'ОШИБКА','No diagnostic data.':'Нет данных диагностики.','CLEAR':'ОЧИСТИТЬ',
'Logs':'Журнал логов','Runtime log stream for diagnosis.':'Поток журнала выполнения для диагностики.','VELORA.LOG':'VELORA.LOG','Waiting…':'Ожидание…',
'No accounts.':'Нет аккаунтов.','No batches.':'Нет заданий.','No events yet.':'Событий пока нет.','Select accounts first':'Сначала выберите аккаунты','Batch id is required':'Укажите ID задания','Batch created':'Задание создано','Map created':'Карта создана','Node saved':'Узел сохранён','Edge connected':'Связь создана','Node deleted':'Узел удалён','Map saved':'Карта сохранена','requested':'запрошено','Kill switch cleared':'Аварийная остановка снята','Emergency stop requested':'Запрос на аварийную остановку отправлен','Visible accounts selected':'Выбраны отображаемые аккаунты','Language':'Язык','Refresh':'Обновить','Confirm':'Подтвердить','Waiting':'Ожидание','connected':'подключено','ready':'готово','accounts':'аккаунтов','account(s)':'аккаунтов','UNKNOWN':'НЕИЗВЕСТНО','No logs.':'Нет логов.',
'NODE':'УЗЕЛ','CONNECTED':'СОЕДИНЕНИЯ','issues':'проблем','nodes':'узлов','edges':'связей',
'OFFLINE':'НЕ В СЕТИ','STARTING':'ЗАПУСК','MENU':'МЕНЮ','QUEUING':'ПОИСК МАТЧА','IN_MATCH':'В МАТЧЕ','STOPPING':'ОСТАНОВКА','ERROR':'ОШИБКА','UNKNOWN':'НЕИЗВЕСТНО','WAITING':'ОЖИДАНИЕ','LIVE':'В ИГРЕ','ROUND_OVER':'РАУНД ЗАВЕРШЁН','GAME_OVER':'МАТЧ ЗАВЕРШЁН','DISABLED':'ОТКЛЮЧЁН','INITIALIZING':'ИНИЦИАЛИЗАЦИЯ','WAITING_FOR_GAME':'ОЖИДАНИЕ ИГРЫ','WAITING_FOR_SPAWN':'ОЖИДАНИЕ ПОЯВЛЕНИЯ','NAVIGATING':'НАВИГАЦИЯ','ARRIVING':'ПРИБЫТИЕ','STUCK':'ЗАСТРЯЛ','RECOVERING':'ВОССТАНОВЛЕНИЕ','REPLANNING':'ПЕРЕПЛАНИРОВАНИЕ','STOPPING':'ОСТАНОВКА','FAULT':'СБОЙ','TRANSITION':'ПЕРЕХОД','Batch started':'Задание запущено','Batch stopped':'Задание остановлено','Batch recovery requested':'Запрошено восстановление задания','Batch deleted':'Задание удалено','Account started':'Аккаунт запущен','Account stopped':'Аккаунт остановлен','Account scheduled':'Аккаунт поставлен в расписание','Account kill requested':'Запрошено завершение аккаунта','Emergency stop completed':'Аварийная остановка выполнена','Emergency stop requested':'Запрошена аварийная остановка','Kill switch cleared':'Аварийная остановка снята','Visible accounts selected':'Выбраны отображаемые аккаунты','Control center refreshed':'Центр управления обновлён','Status API:':'API состояния:','Events:':'События:','Route API:':'API маршрутов:','LOG API ERROR:':'ОШИБКА API логов:','API ERROR':'ОШИБКА API','VALIDATION ERROR':'ОШИБКА ПРОВЕРКИ','Map is required':'Необходимо указать карту','Map already exists':'Карта уже существует','Node saved':'Узел сохранён','Edge connected':'Связь создана','Node deleted':'Узел удалён','Map saved':'Карта сохранена'
}};
...
function applyLanguage(lang){if(lang==='ru')localizeDom()}
...
function stateClass(v){const s=String(v||'').toLowerCase();return s.includes('error')||s.includes('fail')?'bad':s.includes('run')||s.includes('active')?'ok':s.includes('wait')||s.includes('sched')||s.includes('ready')||s.includes('search')?'warn':''}
function selectedAccounts(){return [...document.querySelectorAll('input[data-account]:checked')].map(x=>x.dataset.account)}
function selectVisible(){document.querySelectorAll('#accountsList input[data-account]').forEach(x=>x.checked=true);toast('Visible accounts selected')}
function clearSelection(){document.querySelectorAll('input[data-account]').forEach(x=>x.checked=false)}
function initials(v){return String(v||'?').split(/[ ]+/).map(x=>x[0]).join('').slice(0,2).toUpperCase()}
...
function views(d){lastStatus=d;const a=d.accounts||[],b=d.batches||[];$('lastUpdate').textContent='● '+new Date().toLocaleTimeString();$('mA').textContent=a.length;$('mR').textContent=a.filter(x=>stateClass(x.state)==='ok').length;$('mB').textContent=b.length;$('mW').textContent=a.filter(x=>stateClass(x.walkbot)==='ok').length;const ks=!!d.kill_switch;$('topState').innerHTML='<span class=pulse></span>'+(ks?'KILL SWITCH':'RUNNING');$('topState').className='chip '+(ks?'bad':'');$('sideState').textContent=ks?'KILL SWITCH ACTIVE':d.running?'SYSTEM ONLINE':'SYSTEM STOPPED';$('clearKillBtn').style.display=ks?'inline-block':'none';$('ovAccounts').innerHTML=a.slice(0,8).map(x=>accountRow(x)).join('')||'<div class=muted>No accounts.</div>';renderAccounts(a);renderBatches(b);renderFSM(a);renderMatch(a);renderWalk(a);renderGsi(a);if($('fsmAccount')&&!$('fsmAccount').value&&a.length)$('fsmAccount').value=a[0].id;renderFsmHistory();if($('language').value==='ru')localizeDom()}
function renderBatches(b){$('batchList').innerHTML=b.map(x=>{const p=x.expected_players?Math.min(100,Math.round((x.ready_players||0)/x.expected_players*100)):0;const members=(x.account_ids||[]);const errors=(x.errors||[]);const match=esc(x.match_id||'—');const memberStates=members.map(id=>{const a=(lastStatus?.accounts||[]).find(v=>v.id===id);return '<span class="state '+(a?stateClass(a.state):'')+'">'+esc(a?a.state:'UNKNOWN')+'</span>'}).join(' ');return '<div class=batch><div><b>'+esc(x.id)+'</b><div class=muted>'+esc(x.mode)+'</div></div><div><b>'+esc(x.state)+'</b><div class=progress><i style="width:'+p+'%"></i></div><small class=muted>'+p+'% ready · match '+match+'</small><div style="margin-top:6px">'+memberStates+'</div>'+(errors.length?'<div class=bad style="margin-top:5px">ERROR: '+esc(String(errors[errors.length-1]))+'</div>':'')+'</div><div class=cell>'+x.size+' accounts</div><div class=cell>'+(x.ready_players||0)+' / '+(x.expected_players||0)+' ready</div><div class=actions-sm><button onclick="batchAct(this.dataset.id,&quot;start&quot;)" data-id="'+esc(x.id)+'">START</button><button onclick="batchAct(this.dataset.id,&quot;recover&quot;)" data-id="'+esc(x.id)+'">RECOVER</button><button class=danger onclick="batchAct(this.dataset.id,&quot;stop&quot;)" data-id="'+esc(x.id)+'">STOP</button><button onclick="batchAct(this.dataset.id,&quot;delete&quot;)" data-id="'+esc(x.id)+'">DELETE</button></div></div>'}).join('')||'<div class=muted>No batches.</div>'}
function renderFSM(a){const g={};a.forEach(x=>g[x.state]=(g[x.state]||0)+1);const sel=$('fsmAccount');if(sel){const current=sel.value;sel.innerHTML=a.map(x=>'<option value="'+esc(x.id)+'">'+esc(x.name)+'</option>').join('');if(a.some(x=>x.id===current))sel.value=current} $('fsmBoard').innerHTML=Object.entries(g).map(([k,v])=>'<div><small class=muted>ACCOUNT STATE</small><b>'+esc(k)+'</b><small class=muted>'+v+' account(s)</small></div>').join('')||'<div class=muted>No states.</div>';$('fsmTable').innerHTML=a.map(x=>'<tr><td><b>'+esc(x.name)+'</b></td><td>'+esc(x.state)+'</td><td>'+x.restart_count+'</td><td>'+esc(x.process_id||'—')+'</td><td>'+esc(x.started_at||'—')+'</td></tr>').join('')}\nfunction renderFsmHistory(){const id=$('fsmAccount')?.value;const type=$('fsmHistoryType')?.value||'fsm';const a=(lastStatus?.accounts||[]).find(x=>x.id===id);const h=a?(a[type+'_history']||[]):[];$('fsmHistory').innerHTML=h.length?h.slice().reverse().map(x=>'<div class=acct><div class=acctmain><div class=avatar>→</div><div><b>'+esc(x.source)+' → '+esc(x.target)+'</b><small>'+esc(x.event)+' · '+new Date(x.ts*1000).toLocaleTimeString()+'</small></div></div><span class=state>TRANSITION</span></div>').join(''):'<div class=muted>No transition history.</div>';if($('language').value==='ru')localizeDom()}
...
function renderWalk(a){$('walkBoard').innerHTML=a.map(x=>{const t=x.walkbot_telemetry||{};const recovery=t.recovery_active?'RECOVERY ACTIVE':'NORMAL';const decision=t.decision_action?String(t.decision_action)+' · '+String(t.decision_reason||''):'—';return '<div class=acct><div class=acctmain><div class=avatar>'+initials(x.name)+'</div><div><b>'+esc(x.name)+'</b><small>'+esc(x.id)+'</small></div></div><span class="state '+stateClass(x.walkbot)+'">● '+esc(x.walkbot)+'</span><div class=cell>ROUTE<b>'+esc(x.route_map||'—')+'</b></div><div class=cell>TARGET<b>'+esc(t.target_node||x.route_goal||'—')+'</b></div><div class=cell>DECISION<b>'+esc(decision)+'</b></div><div class=cell>DISTANCE<b>'+esc(t.distance_to_target==null?'—':String(t.distance_to_target))+'</b></div><div class=cell>GSI<b>'+((x.gsi_age==null)?'OFFLINE':x.gsi_age.toFixed(1)+'s')+'</b></div><div class="cell '+(t.recovery_active?'bad':'ok')+'"><b>'+recovery+'</b></div><div class=actions-sm><button class=danger onclick="act(this.dataset.id,&quot;kill&quot;)" data-id="'+esc(x.id)+'">KILL</button></div></div>'}).join('')||'<div class=muted>No WalkBot workers.</div>'; $('walkTelemetry').innerHTML=a.map(x=>{const t=x.walkbot_telemetry||{};return '<div class=health-item><b>'+esc(x.name)+'</b><span>state='+esc(t.state||'—')+' · area='+esc(t.current_nav_area||'—')+' · target='+esc(t.navigation_goal||t.target_node||'—')+' · confidence='+esc(t.position_confidence==null?'—':String(t.position_confidence))+' · recovery='+esc(t.recovery_level==null?'—':String(t.recovery_level))+' · vision='+esc(t.vision_confidence==null?'—':String(t.vision_confidence))+'</span></div>'}).join('')||'<div class=muted>No WalkBot telemetry.</div>'}}function renderGsi(a){const g=(lastStatus&&lastStatus.gsi)||{},gh=(lastStatus&&lastStatus.gsi_health)||{},on=a.filter(x=>x.gsi_age!=null&&x.gsi_age<10).length;const server=g.connected&&!g.stale?'ONLINE':g.connected?'STALE':'OFFLINE';$('gsiMetrics').innerHTML='<div class=metric><label>SERVER</label><strong>'+server+'</strong><small>GSI connection</small></div><div class=metric><label>PACKETS</label><strong>'+(g.packet_count||0)+'</strong><small>received packets</small></div><div class=metric><label>ERRORS</label><strong>'+(g.error_count||0)+'</strong><small>packet errors</small></div><div class=metric><label>STREAMS</label><strong>'+(g.stream_count||on)+'</strong><small>active streams</small></div>';$('gsiTable').innerHTML=a.map(x=>'<tr><td><b>'+esc(x.name)+'</b></td><td class="'+(x.gsi_age!=null&&x.gsi_age<10?'ok':'bad')+'">'+(x.gsi_age==null?'OFFLINE':x.gsi_age.toFixed(1)+'s')+'</td><td>'+esc(x.process_id||'—')+'</td><td>'+x.round+'</td><td>'+x.score+'</td><td>'+x.opponent_score+'</td></tr>').join('');if($('language').value==='ru')localizeDom()}
...
async function diag(){try{lastDiag=await api('/api/diagnostics');$('diagRaw').textContent=JSON.stringify(lastDiag,null,2);const rows=Array.isArray(lastDiag)?lastDiag:Object.entries(lastDiag||{}).map(([name,v])=>({name,ok:!!(v&&v.ok),detail:v&&v.detail||String(v??'')}));$('diagGrid').innerHTML=rows.map(v=>'<div><small>'+esc(String(v.name||'CHECK').replaceAll('_',' ').toUpperCase())+'</small><b class="'+(v.ok?'ok':'bad')+'">'+(v.ok?'● PASS':'● FAIL')+'</b><small style="display:block;margin-top:5px">'+esc(v.detail||'')+'</small></div>').join('')||'<div class=muted>No diagnostic data.</div>';$('healthGrid').innerHTML=$('diagGrid').innerHTML;if($('language').value==='ru')localizeDom()}catch(e){$('diagRaw').textContent=e.message;clientError(e.stack||e.message)}}
function clearDiag(){$('diagRaw').textContent='—';$('diagGrid').innerHTML=''}
async function loadLogs(){try{const j=await api('/api/logs?lines=500');$('logsBox').textContent=j.content||'No logs.';$('logMeta').textContent=(j.lines||500)+' lines · '+(j.path||'');if($('language').value==='ru')localizeDom()}catch(e){$('logsBox').textContent='LOG API ERROR: '+e.message}}
...
async function refreshMap(){if(!$('map').value)return;route=await api('/api/routes/'+encodeURIComponent($('map').value));renderRoute();await validateRoute()}
async function validateRoute(){try{const j=await api('/api/routes/'+encodeURIComponent($('map').value)+'/validate');const el=$('routeValidation');if(!el)return;if(j.valid){el.textContent='● VALID · '+j.node_count+' nodes · '+j.edge_count+' edges';el.className='state ok'}else{el.textContent='● INVALID · '+j.problems.length+' issues';el.className='state bad';el.title=j.problems.join('\\n')}}catch(e){$('routeValidation').textContent='VALIDATION ERROR';$('routeValidation').className='state bad'}}
   ...
   def do_GET(self):
    ...
    if p.startswith("/api/routes/"):
     ...
     if len(parts)==4 and parts[3]=="validate":
      return self._json(outer.s.route_store.get(parts[2]).analysis())
    ...
    try:
     ...
     if len(parts)==4 and parts[:2]==["api","routes"]:
      ...
      if parts[3]=="nodes":
       ...
       validation=g.analysis()
       if not validation["valid"]:
        return self._json({"error":"route validation failed","validation":validation["problems"]},400)
       outer.s.route_store.save(map_name,g);return self._json({"ok":True,"validation":[]})
