...
'CONTROL':'УПРАВЛЕНИЕ','MANAGEMENT':'МЕНЕДЖМЕНТ','FARM CONTROL CENTER':'ЦЕНТР УПРАВЛЕНИЯ ФАРМИНГОМ','CONTROL PLANE':'ЦЕНТР УПРАВЛЕНИЯ','Command Center':'Командный центр',
...
'EMERGENCY CONTROL':'АВАРИЙНОЕ УПРАВЛЕНИЕ','Global kill switch and recovery controls':'Глобальная остановка и средства восстановления','KILL SWITCH':'АВАРИЙНАЯ ОСТАНОВКА',
'STOP ALL':'ОСТАНОВИТЬ ВСЁ','Emergency Stop All':'Остановить всё','Immediately stop active batches and automation workers.':'Немедленно остановить активные задания и процессы автоматизации.',
...
'CREATE BATCH':'СОЗДАТЬ ЗАДАНИЕ','SAVE MAP':'СОХРАНИТЬ КАРТУ','RAW OUTPUT':'РЕЗУЛЬТАТ ПРОВЕРКИ','CANCEL':'ОТМЕНА','CONFIRM':'ПОДТВЕРДИТЬ',
'START':'ЗАПУСТИТЬ','STOP':'ОСТАНОВИТЬ','KILL':'ЗАВЕРШИТЬ','RECOVER':'ВОССТАНОВИТЬ',
'Accounts':'Аккаунты','Manage account workers, routes and process lifecycle.':'Управление воркерами аккаунтов, маршрутами и жизненным циклом процессов.',
'All states':'Все состояния','Running':'Работает','Error':'Ошибка','Stopped':'Остановлен','Manual':'Вручную','2v2 Random':'2v2 — случайный','5v5 Shuffle':'5v5 — случайный','Deathmatch':'Бой насмерть','Arms Race':'Гонка вооружений','Armory':'Оружейная',
...
'ACCOUNT LIFECYCLE':'ЖИЗНЕННЫЙ ЦИКЛ АККАУНТА','Live snapshot':'Текущее состояние','STATE TABLE':'ТАБЛИЦА СОСТОЯНИЙ','ACCOUNT':'АККАУНТ','STATE':'СОСТОЯНИЕ','RESTARTS':'ПЕРЕЗАПУСКИ','PID':'PID','STARTED':'ЗАПУЩЕН',
'Match FSM':'FSM матчей','Match search, gameplay and result telemetry.':'Поиск матча, игровой процесс и телеметрия результата.',
'ROUND':'РАУНД','SCORE':'СЧЁТ','RESULT':'РЕЗУЛЬТАТ','No match data.':'Нет данных о матчах.',
'WalkBot':'WalkBot','Navigation state, route target and telemetry age.':'Состояние навигации, цель маршрута и давность телеметрии.','ROUTE':'МАРШРУТ','GOAL':'ЦЕЛЬ','GSI':'GSI','OFFLINE':'НЕ В СЕТИ','No WalkBot workers.':'Нет активных воркеров WalkBot.',
'Game State Integration':'Игровая телеметрия','Connection age and process telemetry for every account.':'Давность подключения и телеметрия процесса для каждого аккаунта.','GSI ONLINE':'GSI В СЕТИ','GSI OFFLINE':'GSI НЕ В СЕТИ','telemetry streams':'потоков телеметрии','no recent event':'нет свежих событий','PROCESS PIDS':'PID ПРОЦЕССОВ','active process handles':'активных процессов','ROUND EVENTS':'СОБЫТИЯ РАУНДОВ','observed rounds':'зафиксированных раундов','OPPONENT':'СОПЕРНИК',
'Route Editor':'Редактор маршрутов','Build and validate WalkBot navigation graphs.':'Создание и проверка графов навигации WalkBot.','NEW MAP':'НОВАЯ КАРТА','ROUTE GRAPH':'ГРАФ МАРШРУТА','TOOLS':'ИНСТРУМЕНТЫ','Add waypoint':'Добавить точку','Connect nodes':'Соединить узлы','Delete node':'Удалить узел','NODE ID':'ID УЗЛА','X / Y / Z':'X / Y / Z','ADD / UPDATE NODE':'ДОБАВИТЬ / ОБНОВИТЬ УЗЕЛ','NODE INSPECTOR':'ИНСПЕКТОР УЗЛА','Select a node.':'Выберите узел.','CONNECT FROM':'ИЗ УЗЛА','CONNECT TO':'В УЗЕЛ','CONNECT':'СОЕДИНИТЬ','DELETE NODE':'УДАЛИТЬ УЗЕЛ','ROUTE GRAPH':'ГРАФ МАРШРУТА','NO NODES · ADD WAYPOINT':'НЕТ УЗЛОВ · ДОБАВЬТЕ ТОЧКУ',
'Diagnostics':'Диагностика','Runtime, Steam, CS2, ports and data directory checks.':'Проверка среды выполнения, Steam, CS2, портов и каталога данных.','SYSTEM':'СИСТЕМА','PASS':'ПРОЙДЕНО','FAIL':'ОШИБКА','No diagnostic data.':'Нет данных диагностики.',
'Logs':'Логи','Runtime log stream for diagnosis.':'Поток журналов выполнения для диагностики.','VELORA.LOG':'VELORA.LOG','Waiting…':'Ожидание…',
'No accounts.':'Нет аккаунтов.','No batches.':'Нет заданий.','Select accounts first':'Сначала выберите аккаунты','Batch id is required':'Укажите ID задания','Batch created':'Задание создано','Map created':'Карта создана','Node saved':'Узел сохранён','Edge connected':'Связь создана','Node deleted':'Узел удалён','Map saved':'Карта сохранена','requested':'запрошено','Kill switch cleared':'Аварийная остановка снята','Emergency stop requested':'Запрос на аварийную остановку отправлен',
'Language':'Язык','Refresh':'Обновить','Confirm':'Подтвердить','CLEAR':'ОЧИСТИТЬ','Waiting':'Ожидание','connected':'подключено','ready':'готово','accounts':'аккаунтов','account(s)':'аккаунт(ов)'
}};
function t(s){const v=I18N.ru[String(s)];return v||String(s)}
function localizeDom(){if(!I18N.ru)return;const walk=n=>{n.childNodes.forEach(ch=>{if(ch.nodeType===3){const raw=ch.nodeValue||'',trim=raw.trim();if(I18N.ru[trim])ch.nodeValue=raw.replace(trim,I18N.ru[trim]);else if(/^\\d+ accounts$/.test(trim))ch.nodeValue=trim.replace(/accounts$/,'аккаунтов');else if(/^\\d+ account\\(s\\)$/.test(trim))ch.nodeValue=trim.replace(/account\\(s\\)$/,'аккаунт(ов)');else if(/^\\d+ [/] \\d+ ready$/.test(trim))ch.nodeValue=trim.replace(/ready$/,'готово')}else if(ch.nodeType===1&&ch.id!=='language'&&ch.tagName!=='SCRIPT'&&ch.tagName!=='STYLE'){['placeholder','title','aria-label'].forEach(a=>{if(ch.hasAttribute(a)){const v=ch.getAttribute(a);if(I18N.ru[v])ch.setAttribute(a,I18N.ru[v])}});walk(ch)}})};walk(document.body)}
