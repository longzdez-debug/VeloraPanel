...
class DecisionEngine:
    ...
    def decide(self, world: WorldSnapshot, goal: NavigationGoal | None = None, max_localization_age: float = 3.0) -> Decision:
        ...
        if not loc.position.fresh(max_age=max_localization_age):
            return Decision("relocalize", None, "localization_stale", min(loc.position.confidence, 0.25))
        if goal is not None:
