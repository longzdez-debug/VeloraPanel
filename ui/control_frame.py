...
class ControlFrame(customtkinter.CTkFrame):
    def __init__(self, parent):
        ...
        data = [
            ("Разложить окна CS2", None, self.move_all_cs_windows),
            ("Остановить процессы", "red", self.kill_all_cs_and_steam),
            ("Запустить BES", "darkgreen", self.launch_bes),
            ("Запустить SRT", "darkgreen", self.launch_srt),
            ("Поддержать разработчика", "darkgreen", self.sendCasesMe),
        ]
    ...
    def kill_all_cs_and_steam(self):
        """Stop only Steam/CS2 processes owned by VELORA accounts."""
        print("🛑 Останавливаю только процессы VELORA...")
        killed = 0
        ...
        try:
            ...
            for acc in account_manager.accounts:
                owned = []
                for process in (getattr(acc, "CS2Process", None), getattr(acc, "steamProcess", None)):
        ...
        print(f"✅ Остановлено процессов VELORA: {killed}")
        if self.accounts_list_frame:
            self.accounts_list_frame.update_label()

    def _clear_steam_userdata(self):
        settings_manager = SettingsManager()
        steam_path = settings_manager.get("SteamPath", r"C:\\Program Files\\Steam\\steam.exe")
