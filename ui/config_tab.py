...
from core.steam_paths import discover_cs2_path, discover_steam_path
from core.steam_safety import steam_safety
...
class ConfigTab(customtkinter.CTkTabview):
    def __init__(self, parent):
        ...
        b1 = customtkinter.CTkButton(
            self.tab("Config"),
            text="Путь к Steam",
            command=lambda: self.set_path("SteamPath", "Steam", str(discover_steam_path() or "C:/Program Files/Steam/steam.exe")),
        )
        b2 = customtkinter.CTkButton(
            ...
            command=lambda: self.set_path(
                "CS2Path",
                "CS2",
                str(discover_cs2_path() or "C:/Program Files/Steam/steamapps/common/Counter-Strike Global Offensive"),
            ),
        )
    ...
    def load_settings(self):
        ...
        steam_path = self._settingsManager.get("SteamPath", "C:/Program Files/Steam/steam.exe")
        if steam_path:
            print(f"Loaded SteamPath: {steam_path}")

        cs2_path = self._settingsManager.get(
            "CS2Path", "C:/Program Files/Steam/steamapps/common/Counter-Strike Global Offensive"
        )
        if cs2_path:
    ...
    def send_trade_selected(self, on_trade_sent=None):
        ...
        dry_run = bool(self._settingsManager.get("LooterDryRun", True))
        try:
            max_items = max(0, int(self._settingsManager.get("LooterMaxItems", 0) or 0))
        except (TypeError, ValueError):
            max_items = 0
        threading.Thread(
            target=self._send_trade_worker,
            args=(selected_accounts, trade_link, inventory_string, script_path, on_trade_sent, dry_run, max_items),
            daemon=True,
        ).start()
    ...
    def _send_trade_worker(
        ...
        for acc in selected_accounts:
            steam_safety.wait(
                "trade",
                acc.login,
                int(self._settingsManager.get("SteamTradeMinInterval", 45)),
            )
            ...
            payload = json.dumps(
                {
                    ...
                    "tradeOfferLink": trade_link,
                    "inventoryString": inventory_string,
                    "dryRun": bool(dry_run),
            ...
            if result.returncode == 0:
                ...
                action = "dry-run inventory check" if dry_run else "send trade"
                self._logManager.add_log(f"{acc.login} {action}: {sent_count} items")
                ...
                if self._is_authorization_error(error_line):
                    steam_safety.block(
                        acc.login,
                        int(self._settingsManager.get("SteamAuthCooldown", 900)),
                    )
