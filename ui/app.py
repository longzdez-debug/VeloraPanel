...
from core.resource_monitor import collect as collect_resource_snapshot
...
TXT_SOFT = "#C2CCC4"
ACCENT_BLUE = "#9B7CFF"
ACCENT_BLUE_DARK = "#7658E8"
ACCENT_GREEN = "#B8F36B"
ACCENT_RED = "#FF6B6B"
ACCENT_PURPLE = "#2A204A"
ACCENT_ORANGE = "#F4B860"
FONT_UI = "Segoe UI"
...
class App(customtkinter.CTk):
    def __init__(self, gsi_manager=None, startup_gpu_info=None, startup_services_initializer=None):
        super().__init__()
        self.title("VELORA / Operations Console")
    ...
    def _create_splash_screen(self):
        ...
        self.splash_window.title("VELORA PANEL")
    ...
    def _build_layout(self):
        ...
        customtkinter.CTkLabel(
            self.sidebar,
            text="VELORA",
            font=customtkinter.CTkFont(family=FONT_UI, size=26, weight="bold"),
            text_color=TXT_MAIN,
        ).grid(row=0, column=0, padx=22, pady=(26, 0), sticky="w")
        customtkinter.CTkLabel(
            self.sidebar,
            text="STEAM  /  CS2  /  OPERATIONS",
            text_color=ACCENT_BLUE,
            font=customtkinter.CTkFont(size=10, weight="bold"),
        ).grid(row=1, column=0, padx=20, pady=(0, 20), sticky="w")
        customtkinter.CTkLabel(
            self.sidebar,
            text="VELORA 2.1  •  RU EDITION",
            text_color=ACCENT_GREEN,
            font=customtkinter.CTkFont(family=FONT_MONO, size=10, weight="bold"),
        ...
        for idx, (key, text) in enumerate(nav_items, start=3):
            btn = customtkinter.CTkButton(
                self.sidebar,
                text={"Functionals": "Главная", "Configurations": "Настройки", "Accs Statistics": "Статистика"}[text],
                width=220,
                height=48,
                corner_radius=12,
                font=customtkinter.CTkFont(family=FONT_UI, size=13, weight="bold"),
        ...
        logs_wrap.grid(row=8, column=0, padx=14, pady=(8, 14), sticky="nsew")
        ...
        self.content.grid(row=0, column=1, padx=(8, 16), pady=16, sticky="nsew")
    ...
    def _build_functional_section(self, parent):
        ...
        customtkinter.CTkLabel(
            title_frame,
            text="Операционный центр",
            text_color=TXT_MAIN,
            font=customtkinter.CTkFont(family=FONT_UI, size=30, weight="bold"),
        ).grid(row=0, column=0, padx=(0, 16), sticky="w")

        customtkinter.CTkLabel(
            title_frame,
            text="Аккаунты, процессы и автоматизация — в одном рабочем пространстве",
            text_color=TXT_MUTED,
            font=customtkinter.CTkFont(family=FONT_UI, size=12),
        ).grid(row=1, column=0, pady=(3, 0), sticky="w")

        self.accounts_info = customtkinter.CTkLabel(
            title_frame,
            text="0 аккаунтов  •  0 выбрано  •  0 запущено",
            text_color=TXT_MUTED,
            font=customtkinter.CTkFont(family=FONT_MONO, size=11),
        )
        self.accounts_info.grid(row=0, column=1, rowspan=2, padx=(26, 0), sticky="e")

        self.health_info = customtkinter.CTkLabel(
            title_frame,
            text="СИСТЕМА: ОЖИДАНИЕ",
            text_color=TXT_MUTED,
            font=customtkinter.CTkFont(family=FONT_MONO, size=11, weight="bold"),
        )
        self.health_info.grid(row=0, column=2, rowspan=2, padx=(18, 0), sticky="e")

        search_wrap = customtkinter.CTkFrame(title_frame, fg_color="transparent")
        search_wrap.grid(row=0, column=3, rowspan=2, padx=(20, 0), sticky="e")
        ...
        customtkinter.CTkEntry(
            ...
            placeholder_text="Поиск аккаунта",
            width=240,
            height=38,
            fg_color=BG_CARD,
            border_color=BG_BORDER,
        ...
        actions.grid(row=1, column=0, padx=18, pady=(0, 12), sticky="ew")
        for i in range(5):
            actions.grid_columnconfigure(i, weight=1)

        common_btn = {"height": 42, "font": customtkinter.CTkFont(family=FONT_UI, size=12, weight="bold"), "corner_radius": 10}
        customtkinter.CTkButton(actions, text="Запустить выбранные", command=self._action_start_selected, fg_color=ACCENT_BLUE, hover_color=ACCENT_BLUE_DARK, **common_btn).grid(row=0, column=0, padx=6, pady=8, sticky="ew")
        customtkinter.CTkButton(actions, text="Выбрать 4", command=self._action_select_first_4, fg_color=ACCENT_PURPLE, hover_color="#3B3568", **common_btn).grid(row=0, column=1, padx=6, pady=8, sticky="ew")
        customtkinter.CTkButton(actions, text="Выбрать все", command=self._action_select_all_toggle, fg_color=BG_CARD_ALT, hover_color=BG_BORDER, **common_btn).grid(row=0, column=2, padx=6, pady=8, sticky="ew")
        customtkinter.CTkButton(actions, text="Остановить выбранные", command=self._action_kill_selected, fg_color=BG_CARD_ALT, hover_color=BG_BORDER, **common_btn).grid(row=0, column=3, padx=6, pady=8, sticky="ew")
        customtkinter.CTkButton(actions, text="+ Добавить аккаунт", command=self._open_add_account_dialog, fg_color=ACCENT_GREEN, hover_color="#2F9F6A", **common_btn).grid(row=0, column=4, padx=6, pady=8, sticky="ew")

        main = customtkinter.CTkFrame(frame, fg_color="transparent")
        main.grid(row=2, column=0, padx=18, pady=(0, 16), sticky="nsew")
        ...
        accounts_block.grid_rowconfigure(2, weight=1)
        accounts_block.grid_columnconfigure(0, weight=1)
        customtkinter.CTkLabel(accounts_block, text="Ферма", font=customtkinter.CTkFont(family=FONT_UI, size=20, weight="bold"), text_color=TXT_MAIN).grid(row=0, column=0, padx=14, pady=(14, 4), sticky="w")

        table_head = customtkinter.CTkFrame(accounts_block, fg_color="transparent")
        table_head.grid(row=1, column=0, padx=12, pady=(0, 3), sticky="ew")
        table_head.grid_columnconfigure(0, weight=1)
        table_head.grid_columnconfigure(1, weight=0, minsize=90)
        table_head.grid_columnconfigure(2, weight=0, minsize=90)
        table_head.grid_columnconfigure(3, weight=0, minsize=100)
        for column, text in enumerate(("АККАУНТ", "LVL", "XP", "СТАТУС")):
            customtkinter.CTkLabel(
                table_head,
                text=text,
                text_color=TXT_MUTED,
                font=customtkinter.CTkFont(family=FONT_MONO, size=9, weight="bold"),
                anchor="w",
            ).grid(row=0, column=column, padx=5, sticky="w")

        self.accounts_scroll = customtkinter.CTkScrollableFrame(accounts_block, fg_color=BG_CARD_ALT)
        self.accounts_scroll.grid(row=2, column=0, padx=8, pady=(0, 8), sticky="nsew")
        self.accounts_scroll.grid_columnconfigure(0, weight=1)
        self._create_account_rows()

        farm_bar = customtkinter.CTkFrame(accounts_block, fg_color="#121016", corner_radius=10, border_width=1, border_color="#302943")
        farm_bar.grid(row=3, column=0, padx=10, pady=(0, 10), sticky="ew")
        farm_bar.grid_columnconfigure(0, weight=1)
        farm_bar.grid_columnconfigure(1, weight=1)
        customtkinter.CTkComboBox(
            farm_bar,
            values=["Матчмейкинг", "Премьер", "Wingman"],
            height=32,
            border_color="#302943",
            fg_color="#1A1721",
            button_color=ACCENT_BLUE,
            dropdown_fg_color=BG_CARD,
        ).grid(row=0, column=0, padx=(10, 4), pady=8, sticky="ew")
        customtkinter.CTkComboBox(
            farm_bar,
            values=["Deathmatch", "Casual", "Competitive"],
            height=32,
            border_color="#302943",
            fg_color="#1A1721",
            button_color=ACCENT_BLUE,
        ...
        customtkinter.CTkButton(
            farm_bar, text="Забрать награды", height=32, fg_color="#1A1721",
            hover_color=BG_BORDER, command=self._action_send_trade_selected,
        ).grid(row=0, column=2, padx=4, pady=8)
        customtkinter.CTkButton(
            farm_bar, text="Запустить ферму", height=32, fg_color=ACCENT_BLUE,
            hover_color=ACCENT_BLUE_DARK, command=self._action_start_selected,
        ).grid(row=0, column=3, padx=4, pady=8)
        customtkinter.CTkButton(
            farm_bar, text="Стоп", width=68, height=32, fg_color="#1A1721",
            hover_color="#38212B", command=self._action_kill_selected,
        ...
        customtkinter.CTkLabel(self.srt_placeholder, text="Маршрутизация Steam", text_color=ACCENT_GREEN, font=customtkinter.CTkFont(family=FONT_UI, size=14, weight="bold")).grid(row=0, column=0, padx=8, pady=(8, 3), sticky="w")
        ...
        customtkinter.CTkButton(actions_bar, text="Заблокировать всё", fg_color=ACCENT_RED, hover_color="#B63D52", height=28, command=self._srt_block_all, font=customtkinter.CTkFont(family=FONT_UI, size=11, weight="bold")).grid(row=0, column=0, padx=(0, 4), sticky="ew")
        customtkinter.CTkButton(actions_bar, text="Сбросить", fg_color=BG_CARD_ALT, hover_color=BG_BORDER, height=28, command=self._srt_reset, font=customtkinter.CTkFont(family=FONT_UI, size=11, weight="bold")).grid(row=0, column=1, padx=(4, 0), sticky="ew")
        ...
        customtkinter.CTkLabel(tools, text="Быстрые действия", text_color=TXT_MAIN, font=customtkinter.CTkFont(family=FONT_UI, size=16, weight="bold")).grid(row=0, column=0, padx=8, pady=(8, 6), sticky="w")
        extra_buttons = [
            ("Разложить окна CS2", self._action_move_all_cs_windows, BG_CARD_ALT),
            ("Остановить процессы", self._action_kill_all_cs_and_steam, ACCENT_PURPLE),
            ("Отправить трейд", self._action_send_trade_selected, ACCENT_GREEN),
            ("Настройки трейда", self._action_open_looter_settings, ACCENT_RED),
            ("Пометить фарм", self._action_marked_farmer, ACCENT_ORANGE),
        ...
        customtkinter.CTkLabel(lobby, text="Управление лобби", text_color=TXT_MAIN, font=customtkinter.CTkFont(family=FONT_UI, size=13, weight="bold")).grid(row=0, column=0, columnspan=2, padx=8, pady=(8, 4), sticky="w")
        for i in range(2):
    ...
    def _start_runtime_status_tracking(self):
        def poll():
            try:
                ...
                self.resource_snapshot = collect_resource_snapshot(self.account_manager.accounts)
                if hasattr(self, "health_info"):
                    snapshot = self.resource_snapshot
                    color = "#2ee66f" if snapshot.memory_percent < 85 else "#ff9500"
                    self.health_info.configure(
                        text=(
                            f"СИСТЕМА: {snapshot.cs2_count} CS2 / "
                            f"{snapshot.steam_count} Steam • "
    ...
    def _update_accounts_info(self):
        ...
        if hasattr(self, "accounts_info"):
            self.accounts_info.configure(text=f"{total} аккаунтов  •  {selected} выбрано  •  {launched} запущено")
    ...
    def _build_config_section(self, parent):
        ...
        customtkinter.CTkLabel(header, text="Настройки", font=customtkinter.CTkFont(family=FONT_UI, size=24, weight="bold"), text_color=TXT_MAIN).grid(row=0, column=0, sticky="w")
        ...
        customtkinter.CTkLabel(switches_card, text="Автоматизация", text_color=TXT_MAIN, font=customtkinter.CTkFont(family=FONT_UI, size=15, weight="bold")).grid(row=0, column=0, padx=10, pady=(8, 2), sticky="w")

        self.config_toggle_auto_accept = self._create_labeled_switch(
            switches_card,
            row=1,
            title="Автопринятие матча",
        ...
        customtkinter.CTkLabel(paths_card, text="Пути Steam / CS2", text_color=TXT_MAIN, font=customtkinter.CTkFont(family=FONT_UI, size=15, weight="bold")).grid(row=0, column=0, padx=10, pady=(8, 2), sticky="w")
    ...
    def _open_add_account_dialog(self):
        ...
        customtkinter.CTkLabel(
            dialog,
            text="Новый Steam-аккаунт",
            text_color=TXT_MAIN,
            font=customtkinter.CTkFont(size=20, weight="bold"),
        ).grid(row=0, column=0, padx=24, pady=(22, 4), sticky="w")
        customtkinter.CTkLabel(
            dialog,
            text="Данные сохраняются в зашифрованное DPAPI-хранилище Windows.",
            text_color=TXT_MUTED,
            wraplength=400,
            justify="left",
        ...
        fields = {}
        for index, (label, placeholder, show) in enumerate(
            (
                ("Логин Steam *", "username", ""),
                ("Пароль Steam *", "password", "*"),
                ("Shared Secret (необязательно)", "shared secret", ""),
                ("SteamID (необязательно)", "7656119...", ""),
                ("Identity Secret (необязательно)", "identity secret", ""),
            ),
            start=0,
        ):
            row = 2 + index * 2
            customtkinter.CTkLabel(dialog, text=label, text_color=TXT_SOFT).grid(
                row=row, column=0, padx=24, pady=(5, 2), sticky="w"
            )
            entry = customtkinter.CTkEntry(
                dialog, placeholder_text=placeholder, show=show, height=34
            )
            entry.grid(row=row + 1, column=0, padx=24, pady=(0, 5), sticky="ew")
            fields[label] = entry

        def save_account():
            try:
                account = self.account_manager.add_account(
                    fields["Логин Steam *"].get(),
                    fields["Пароль Steam *"].get(),
                    fields["Shared Secret (необязательно)"].get(),
                    fields["SteamID (необязательно)"].get(),
                    fields["Identity Secret (необязательно)"].get(),
                )
                self._create_account_rows()
                self._safe_ui_refresh()
                self.log_manager.add_log(f"✅ Аккаунт {account.login} добавлен")
                dialog.destroy()
            except (ValueError, OSError, RuntimeError) as exc:
                messagebox.showerror("Не удалось добавить аккаунт", str(exc), parent=dialog)
        ...
        ).grid(row=12, column=0, padx=24, pady=(16, 8), sticky="ew")
        customtkinter.CTkButton(
            dialog,
            text="Отмена",
            height=34,
        ...
        ).grid(row=13, column=0, padx=24, pady=(0, 18), sticky="ew")
    ...
    def _toggle_region_block(self, region):
        def op():
            ...
            target_block_state = not self._is_region_fully_blocked(region)
            server_rules = {
                self._server_rule_name(region, server_ip): server_ip
                for server_ip in self._get_region_servers(region)
            }

            if target_block_state:
                for rule_name, server_ip in server_rules.items():
                    if rule_name not in self.blocked_servers:
                ...
                results = self.route_manager.remove_rules_bulk(server_rules.keys())
                for rule_name, ok in results.items():
                    if ok:
                        self.blocked_servers.discard(rule_name)
    ...
    def _srt_reset(self):
        def op():
            ...
            if self.route_manager is not None:
                self.route_manager.full_cleanup()
    ...
    def _apply_section_switch(self, section_key):
        ...
        for key, button in self.nav_buttons.items():
            ...
            button.configure(fg_color=ACCENT_PURPLE if is_target else BG_CARD_ALT, border_color=ACCENT_GREEN if is_target else BG_BORDER)
        self._pending_section = None
        self._section_switch_job = None
    ...
    def show_section(self, section_key):
        ...
        for k, button in self.nav_buttons.items():
            ...
            button.configure(
                state="normal",
                fg_color=ACCENT_PURPLE if is_selected else BG_CARD_ALT,
                border_color=ACCENT_GREEN if is_selected else BG_BORDER,
            )
    ...
    def _start_telegram_bot(self, token):
        ...
        try:
            ...
            self.telegram_bot_manager = TelegramBotManager(
                ...
                allowed_chat_ids=self._settings_manager.get("TelegramAllowedChatIds", []),
            )
    ...
    def _load_window_position(self):
        try:
            ...
            x = int(parts[0].strip())
            y = int(parts[1].strip())
            width = max(1120, self.winfo_width())
