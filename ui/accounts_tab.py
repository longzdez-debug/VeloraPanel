...
class AccountsControl(customtkinter.CTkTabview):
    def __init__(self, parent, update_label, accounts_list):
        ...
        # Вкладки
        self.add("Управление аккаунтами")
        self.tab("Управление аккаунтами").grid_columnconfigure(0, weight=1)

        self.add("Статистика аккаунтов")
        self.tab("Статистика аккаунтов").grid_columnconfigure(0, weight=1)
    ...
    def create_control_buttons(self):
        buttons = [
            ("Запустить выбранные", "darkgreen", self.start_selected),
            ("Остановить выбранные", "red", self.kill_selected),
            ("Выбрать первые 4", None, self.select_first_4),
            ("Выбрать все", None, self.select_unselect_all_accounts),
            ("Пометить для фарма", "orange", self.mark_farmed),
        ]
        for i, (text, color, cmd) in enumerate(buttons):
            b = customtkinter.CTkButton(self.tab("Управление аккаунтами"), text=text, fg_color=color, command=cmd)
    ...
    def create_stat_buttons(self):
        buttons = [
            ("Получить уровень", None, self.try_get_level),
            ("Ранг Wingman", None, self.try_get_wingmanRank),
            ("Ранги карт", None, self.try_get_mapStats),
            ("Ранг Premier", None, self.try_get_premierRank),
            ("Сохранить HTML-отчёт", None, self.save_stats_to_html),
        ]
        for i, (text, color, cmd) in enumerate(buttons):
            b = customtkinter.CTkButton(self.tab("Статистика аккаунтов"), text=text, fg_color=color,
    ...
    def start_selected(self):
        ...
        steam_path, cs2_path = resolve_paths(
            self._settingsManager.get("SteamPath"),
            self._settingsManager.get("CS2Path"),
        )
        steam_path = str(steam_path or "")
        cs2_path = str(cs2_path or "")
    ...
    def _fetch_html(self, steam, url_suffix="gcpd/730/?tab=matchmaking"):
        try:
            ...
            self._logManager.add_log(f"[{steam.username}] ❌ Failed to login: {e}")
            return None
        try:
    ...
    def save_stats_to_html(self, filename="cs2_stats.html"):
        def worker():
            html_parts = [
                "<!DOCTYPE html><html><head><meta charset='UTF-8'><title>VELORA PANEL - CS2 Stats</title>",
